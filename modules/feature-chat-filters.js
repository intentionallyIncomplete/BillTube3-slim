...
const customFilters = [
  ...
  { name: "giphy v1", source: "https?://media\\d+\\.giphy\\.com/media/v1\\.[^/]+/([^ /\\n]+)/giphy\\.gif", flags: "gi", replace: `<img class="giphy chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="https://media.giphy.com/media/\\1/giphy_s.gif" />`, active: true, filterlinks: true },
  { name: "giphy", source: "https?://media\\d+\\.giphy\\.com/media/(?!v1\\.)([^ /\\n]+)/giphy\\.gif", flags: "gi", replace: `<img class="giphy chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="https://media.giphy.com/media/\\1/giphy_s.gif" />`, active: true, filterlinks: true },
  { name: "giphy i.giphy", source: "https?://i\\.giphy\\.com/([^ /\\n]+)\\.gif", flags: "gi", replace: `<img class="giphy chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="https://media.giphy.com/media/\\1/giphy_s.gif" />`, active: true, filterlinks: true },
  { name: "giphy page", source: "https?://(?:www\\.)?giphy\\.com/gifs/(?:.*-)?([a-zA-Z0-9]+)", flags: "gi", replace: `<img class="giphy chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="https://media.giphy.com/media/\\1/giphy_s.gif" />`, active: true, filterlinks: true },
  { name: "klipy cdn", source: "(https?://static\\d*\\.klipy\\.com/[^\\s<]+)", flags: "gi", replace: `<span class="btfw-klipy-wrap"><img class="klipy chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="\\1" /></span>`, active: true, filterlinks: true },
  { name: "tenor media m", source: "(https?://media\\d*\\.tenor\\.com/m/[^\\s<]+\\.(?:gif|webp))", flags: "gi", replace: `<img class="tenor chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="\\1" />`, active: true, filterlinks: true },
  { name: "tenor", source: "(https?://c\\.tenor\\.com/[\\w-]+/[^\\s<]+\\.(?:gif|webp))", flags: "gi", replace: `<img class="tenor chat-picture chat-media"${CHAT_MEDIA_MAX_STYLE} src="\\1" />`, active: true, filterlinks: true },
  ...
  function unwrapLinksInsideLetterboxdTags(html) {
    return String(html || "").replace(
      /(\[letterboxdcard\][\s\S]*?\[\/letterboxdcard\])/gi,
      (tag) => tag.replace(/<a[^>]*>([^<]*)<\/a>/gi, "$1")
    );
  }
  ...
  async function renderMediaCardsInMessage(span) {
    ...
    if (hadLetterboxd) {
      ...
      try {
        html = unwrapLinksInsideLetterboxdTags(html);
        html = letterboxd.renderCardsInHtml(html);
        span.innerHTML = html;
        await letterboxd.expandSlugTagsInHtml(span);
        html = span.innerHTML;
      } finally {
        delete span.dataset.btfwCardBusy;
      }
    }
  ...
  function processChatMessageDiv(div) {
    ...
    div.querySelectorAll("span:not(.timestamp)").forEach((span) => {
      if (span.classList.contains("username") || span.querySelector(".username")) return;
      void renderMediaCardsInMessage(span);
    });
  }
